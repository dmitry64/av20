...
std::vector<Channel> CalibrationManager::generateChannels(int dispChansNumber)
{
    ...
    for(int i=0; i<8; i++) {
        ...
        for(int j=0; j<dispChansNumber; j++) {
            DisplayChannel dc1 = generateDisplayChannel(i*10);
...
ChannelsCalibration CalibrationManager::generateCalibration(TactID tact, CalibrationIndex index, std::string name, int dispChansNumber)
{
    ...
    info._name = name;
    cal1.setInfo(info);
    const std::vector<Channel> & channels1 = generateChannels(dispChansNumber);
...
{
    ChannelsCalibration cal1 = generateCalibration(TACT_ID_SEARCH_SCHEME_1, 0, "Default calibration", 1);
    addCalibration(cal1);
    ChannelsCalibration cal2 = generateCalibration(TACT_ID_SEARCH_SCHEME_1, 1, "Alt calibration", 1);
    addCalibration(cal2);
